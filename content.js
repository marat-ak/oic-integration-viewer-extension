(function () {
  ...
  /* ── Shared core (iv-core.js) ──────────────────────────────────────
     Constants, blueprint parsing, model helpers and the tree-node
     renderer live in iv-core.js (window.OicIvCore), shared with the
     activity viewer extension's definition panel. The wrappers below
     bind viewer-local state (theme color overrides, xpath preview
     length) into the shared renderer. */
  var Core = window.OicIvCore;
  var THEMES = Core.THEMES;
  var DEFAULT_COLORS = Core.DEFAULT_COLORS;
  var TYPE_DISPLAY = Core.TYPE_DISPLAY;
  var getDisplayType = Core.getDisplayType;
  var isAssignGroup = Core.isAssignGroup;
  var getEffectiveType = Core.getEffectiveType;
  var escapeHtml = Core.escapeHtml;
  var debounce = Core.debounce;
  var parseArchive = Core.parseArchive;
  var parseProjectXml = Core.parseProjectXml;
  var mergeArchiveIntoBlueprint = Core.mergeArchiveIntoBlueprint;
  var getActivityName = Core.getActivityName;
  var getActivityXpath = Core.getActivityXpath;
  var getBreadcrumbLabel = Core.getBreadcrumbLabel;
  var getChildren = Core.getChildren;
  var buildBreadcrumb = Core.buildBreadcrumb;
  var getSearchableText = Core.getSearchableText;
  function truncateXpath(xpath) { return Core.truncateXpath(xpath, maxXpathCharsInPreview); }
  function renderDetail(activity) { return Core.renderDetail(activity); }
  function renderNode(activity, depth, parentPath) {
    return Core.renderNode(activity, depth, parentPath, {
      badgeColor: getBadgeColor,
      maxXpathChars: maxXpathCharsInPreview
    });
  }
  ...
  /* ── Archive download ──────────────────────────────────────────────
     fetchArchive (plain .iar + project .car temp-deployment flow) and
     the consent modal moved to iv-core.js. */
  var fetchArchive = Core.fetchArchive;
